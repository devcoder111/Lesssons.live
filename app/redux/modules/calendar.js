...
const initForm = Map({
  title: '',
  start: moment(new Date()).minute(0).second(0),
  timeStart: moment(new Date()).minute(0).second(0),
  timeEnd: moment(new Date()).minute(0).second(0),
  hexColor: 'EC407A',
});
...
export default function reducer(state = initialImmutableState, action = {}) {
  switch (action.type) {
    ...
    case SUBMIT_EVENT:
      console.log('SUBMIT_EVENT :', action);
      return state.withMutations((mutableState) => {
